...
import { CheckCircle, Users, BookOpen, ArrowLeft, Scale, GraduationCap } from 'lucide-react'
...
export default function WhatIsInclusiveEducation() {
  const educationTypes = [
    ...
    },
    {
      ...
      characteristics: ['مناهج مرنة', 'دعم شامل', 'بيئة متكيفة', 'مشاركة كاملة'],
      ...
      <main className="container-custom section-padding">
        ...
        <nav className="mb-8" aria-label="الرابط التشعبي">
          <ol className="flex items-center space-x-reverse space-x-2 text-sm">
            ...
            <li className="text-gray-600 font-arabic">ما هي التربية الدامجة؟</li>
          </ol>
        </nav>
        ...
        <section className="mb-16">
          <div className="gradient-primary text-white rounded-2xl p-8 md:p-12">
            ...
            <p className="text-xl mb-8 text-blue-50 leading-relaxed font-arabic max-w-4xl">
              التربية الدامجة هي مقاربة تربوية وحقوقية تضمن حق جميع الأطفال في التعلم داخل بيئة تعليمية متكيفة، مع توفير الدعم اللازم لنجاح كل طفل بغض النظر عن قدراته أو خلفيته.
            </p>
            <div className="grid md:grid-cols-2 gap-8">
              ...
              <div className="bg-white/10 backdrop-blur-sm rounded-lg p-6">
                ...
                <p className="text-blue-50 font-arabic leading-relaxed">
                  الإعاقة ليست خاصية فردية، بل نتيجة تفاعل بين الشخص والمحيط والحواجز التي تمنعه من المشاركة الكاملة.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {educationTypes.map((type, index) => (
              <div key={index} className={`border-2 rounded-xl p-6 ${type.color}`}>
                ...
                <div className="mb-4">
                  <h4 className="font-bold font-arabic text-gray-800 mb-2">الخصائص:</h4>
                ...
                {type.limitations && (
                  <div>
                    <h4 className="font-bold font-arabic text-gray-800 mb-2">القيود:</h4>
                    <ul className="space-y-1">
                      {type.limitations.map((limit, i) => (
                        <li key={i} className="flex items-start space-x-reverse space-x-2">
                          <span className="text-red-600 mt-0.5">•</span>
                          <span className="text-gray-700 font-arabic text-sm">{limit}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
                ...
                {type.advantages && (
                  <div>
                    <h4 className="font-bold font-arabic text-gray-800 mb-2">المميزات:</h4>
                    <ul className="space-y-1">
                      {type.advantages.map((advantage, i) => (
                        <li key={i} className="flex items-start space-x-reverse space-x-2">
                          <CheckCircle className="h-4 w-4 text-green-600 mt-0.5 flex-shrink-0" />
                          <span className="text-gray-700 font-arabic text-sm">{advantage}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
                ...
                <ul className="space-y-3">
                  {benefit.items.map((item, i) => (
                    <li key={i} className="flex items-start space-x-reverse space-x-3">
                      ...
                      <span className="text-gray-700 font-arabic leading-relaxed">{item}</span>
                    </li>
                  ))}
                </ul>
              </div>
        ...
        <section className="text-center">
          <div className="bg-gradient-to-r from-primary to-secondary text-white rounded-2xl p-12">
            ...
            <p className="text-xl mb-8 text-blue-50 font-arabic leading-relaxed max-w-2xl mx-auto">
              اكتشف الأدوات والموارد التي ساعدك على تحقيق بيئة تعليمية دامجة لجميع الأطفال
            </p>
